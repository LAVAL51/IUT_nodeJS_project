...
module.exports = [
...

  {
    ...
    handler: async (request, h) => {
      const { moviesService, mailServices, userService } = request.services();
      const response = await moviesService.create(request.payload);
      await mailServices.sendMailNewMovie(request.payload, userService);
      return response;
    }
  },

  {
    ...
    handler: async (request, h) => {
      ...
      await mailServices.sendMailEditedMovie(request.payload, userService);
      return 'Movie edited';
    }
  },
